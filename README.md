...
- 错误与提示可进入独立详情页，查看原始信息、发生环节、影响和建议操作
- 部分读取时提供结构诊断、已读取章节范围、原因详情和重新理解入口
- 支持 X Article 及使用 `div/span` 渲染的 React 长文正文回退提取
- 多个正文候选会选择可读内容最丰富的一项，并补采混合结构中的后续自定义文本
- 选中文字后通过紧凑的页面内快捷按钮提问，或结合文章上下文翻译成中文
- 划词译文直接显示在原文附近，可拖拽避让原文，点击外部区域后关闭
- 引用问答保留多级章节路径；正文引用锁定锚点附近内容，标题引用覆盖完整子章节，且可在发送前取消
...
- 文章证据、会话检查点和对话原文共享统一预算；文章能放入预算时发送全文，超限时融合引用锚点、字面精确匹配、BM25、Planner 证据需求和章节覆盖
- 检索块按段落、句子、代码行、列表项和表格行切分，900 字符是目标值，1,200 字符是硬上限
- 引用用于确定指代和回答重点，不再阻断其他章节与相关对话；历史回答与文章冲突时以本轮文章证据为准
...
- 会话检查点只进入 `browser.storage.session`，不会写入长期学习记录。
- 回答诊断只进入 `browser.storage.session`，当前页面最多保留最近 10 次，归档学习记录时主动剥离。
- 标签页关闭后，删除对应的临时页面上下文。
